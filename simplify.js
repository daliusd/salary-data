    ...
    if (data.length > 11 && data[1].length > 0) {
      ...
      if (year > 2021 || (year == 2021 && month >= 4)) {
        dataByCode[code] = {
          ...
          'm3': extractNumber(data[12]) || undefined, // median
          'f3': extractNumber(data[13]) || undefined, // first 25%
          'l3': extractNumber(data[14]) || undefined, // last 25%
          's3': extractNumber(data[15]) || undefined, // std
        }
      } else {
        dataByCode[code] = {
          'a': extractNumber(data[2]),
